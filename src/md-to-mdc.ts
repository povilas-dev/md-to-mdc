...
export const transformMarkdownLinks = (content: string, currentFilePath: string, baseDir: string,baseOutputPath:string) => {
    ...
    return content.replace(linkRegex, (match, text, url) => {
        ...
        if (url.includes('.md')) {
            ...
            return `[${filePath}](mdc:${baseOutputPath}/${mdcFileName})`;
        }
        return match;
    });
}
...
function createDescription(baseDir:string, inputPath:string) {
  const relativePath = path.relative(baseDir, inputPath);
    ...
    const formattedFileName = path.basename(inputPath, '.md').split('-').map(capitalize).join(' ');
    const directories = relativePath.split(path.sep).filter(el => el !== '..' && el !== trueFileName);
    const formattedDirectories = directories.flatMap(el => el.split('-').map(capitalize)).join(' ');
    // Always include rootDirName at the start
    return `${[capitalize(rootDirName), formattedDirectories].filter(Boolean).join(' ')}: ${formattedFileName}`;
}

function convertFile(inputPath:string, outputPath:string, baseDir:string, baseOutputPath:string) {
    try {
        // Read the input file
        ...
        const transformedContent = transformMarkdownLinks(content, inputPath, baseDir, baseOutputPath);
...
function processDirectory(inputDir:string, outputDir:string) {
    ...
    function processDir(currentPath:string, baseInputPath:string, baseOutputPath:string) {
...
if (stat.isDirectory()) {
    ...
    convertFile(inputPath, outFile, path.dirname(inputPath), path.basename(inputPath));
} else {
